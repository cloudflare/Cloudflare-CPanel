...
current_version=`curl -s https://api.cloudflare.com/host-gw.html -d "act=cpanel_info" -d "host_key=$host_key" | python -mjson.tool | grep cpanel_latest | cut -d "\"" -f 4`
current_version_sha=`curl -s https://api.cloudflare.com/host-gw.html -d "act=cpanel_info" -d "host_key=$host_key" | python -mjson.tool | grep cpanel_sha1 | cut -d "\"" -f 4`
